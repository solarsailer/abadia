...
async function runSelectCommand(inputFile, outputFile, {reset} = options) {
  ...
  const previousData = await getPreviousResult(outputFile)
  const result = await select(data, previousData, {reset})

  writeJsonFile(outputFile, result)
}

async function select(data, previousData, {reset} = options) {
  // No reset flag? Try to find a previous valid result.
  // This allows us to do the process in multiple steps.
  const exportedData = reset ? {} : {...previousData}
